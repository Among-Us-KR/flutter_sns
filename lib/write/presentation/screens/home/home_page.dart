...
final postsStreamProvider = StreamProvider.autoDispose<List<Posts>>((ref) {
  final firestore = FirebaseFirestore.instance;
  final selectedCategory = ref.watch(selectedCategoryProvider);

  Query<Map<String, dynamic>> query = firestore.collection('posts');

  // '전체'가 아니면 카테고리 필터링 추가
  if (selectedCategory != Category.all) {
    query = query.where('category', isEqualTo: selectedCategory.displayName);
  }
  ...
  return query
      ...
      .map(
        (snapshot) =>
            snapshot.docs.map((doc) => postFromFirestore(doc)).toList(),
      );
});
...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  Widget build(BuildContext context) {
    // ConsumerStatefulWidget을 사용하면 `build` 메서드에서 `ref`를 파라미터로 받지 않습니다.
    // 대신, `ConsumerState` 클래스의 프로퍼티로 `ref`에 접근할 수 있습니다.
